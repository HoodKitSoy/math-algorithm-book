...
#
# 各言語で実装された「008-Brute_Force_1」のコードをテストし、実行速度を計測するスクリプト
#
...
# テストケース (入力 => 期待される出力)
# 2つの整数入力（N S、出力は条件を満たす組み合わせの個数）
declare -A TEST_CASES

# テストケース1: 問題例1 (3 4 => 6)
TEST_CASES["case1_input"]="3 4"
TEST_CASES["case1_expected"]="6"

# テストケース2: 問題例2 (869 120 => 7140)
TEST_CASES["case2_input"]="869 120"
TEST_CASES["case2_expected"]="7140"
...
run_tests() {
    ...
    for i in {1..2}; do
        ...
        if [[ "$actual_output" == "$expected_output" ]]; then
            printf "  Test %d: Input=[%-8s] -> Output: %-4s. ${GREEN}PASS${NC} (%sms)\n" "$test_count" "$input_display" "$actual_output" "$exec_time"
        else
            printf "  Test %d: Input=[%-8s] -> Output: %-4s (Expected: %-4s). ${RED}FAIL${NC}\n" "$test_count" "$input_display" "$actual_output" "$expected_output"
            all_passed=false
        fi
    done
...
print_header "Running tests for 008-Brute_Force_1"
