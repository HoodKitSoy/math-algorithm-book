...
#
# 各言語で実装された「004-Product of 3 Integers」のコードをテストし、実行速度を計測するスクリプト
#
...
# テストケース1: 例1 (2 8 8 => 128)
TEST_CASES["case1_input"]="2 8 8"
TEST_CASES["case1_expected"]="128"

# テストケース2: 例2 (7 7 25 => 1225)
TEST_CASES["case2_input"]="7 7 25"
TEST_CASES["case2_expected"]="1225"

# テストケース3: 例3 (100 100 100 => 1000000)
TEST_CASES["case3_input"]="100 100 100"
TEST_CASES["case3_expected"]="1000000"

# テストケース4: 最小値テスト (1 1 1 => 1)
TEST_CASES["case4_input"]="1 1 1"
TEST_CASES["case4_expected"]="1"

# テストケース5: 混合値テスト (3 4 5 => 60)
TEST_CASES["case5_input"]="3 4 5"
TEST_CASES["case5_expected"]="60"
...
run_tests() {
    ...
    for i in {1..5}; do
        ...
        # 入力データを表示用に取得（3つの整数）
        local input_display="$input"
        
        # 結果を検証して表示
        if [[ "$actual_output" == "$expected_output" ]]; then
            printf "  Test %d: Input=\"%-12s\" -> Output: %-7s. ${GREEN}PASS${NC} (%sms)\n" "$test_count" "$input_display" "$actual_output" "$exec_time"
        else
            printf "  Test %d: Input=\"%-12s\" -> Output: %-7s (Expected: %-7s). ${RED}FAIL${NC}\n" "$test_count" "$input_display" "$actual_output" "$expected_output"
            all_passed=false
        fi
    done
...
print_header "Running tests for 004-Product of 3 Integers"
