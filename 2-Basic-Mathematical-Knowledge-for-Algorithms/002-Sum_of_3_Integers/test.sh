...
#
# 各言語で実装された「002-Sum of 3 Integers」のコードをテストし、実行速度を計測するスクリプト
#
...
# テストケース (入力 => 期待される出力)
# キーは引用符で囲んで、スペースを含む文字列を1つのキーとして扱えるようにする
...
run_tests() {
    ...
    for input in "${!TEST_CASES[@]}"; do
        ...
        # 結果を検証して表示
        # printfのフォーマットを調整して長い入力に対応
        if [[ "$actual_output" == "$expected_output" ]]; then
            printf "  Input: \"%-15s\" -> Output: %-5s. ${GREEN}PASS${NC} (%sms)\n" "$input" "$actual_output" "$exec_time"
        else
            printf "  Input: \"%-15s\" -> Output: %-5s (Expected: %-5s). ${RED}FAIL${NC}\n" "$input" "$actual_output" "$expected_output"
            all_passed=false
        fi
    done
...
print_header "Running tests for 002-Sum of 3 Integers"
